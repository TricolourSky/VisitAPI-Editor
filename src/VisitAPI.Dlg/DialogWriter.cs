...
public static class DialogWriter
{
    ...
    static string Trigger(DialogTree t, DialogTrigger g)
    {
        ...
        if (g.Enter >= 0f) sb.Append(" enter ").Append(Num(g.Enter));   // 计时型没坐标
        else sb.Append($" ({Num(g.X)}, {Num(g.Y)}, {Num(g.Z)})");
    ...
    static List<string> Directives(DialogTree t, DialogOption o)
    {
        ...
        if (o.IfItems) d.Add(o.IfItemsId == null ? "ifitems" : $"ifitems: {Alias(t, o.IfItemsId)}");
