...
public static class FileApi
{
    ...
    public static void Map(WebApplication app, Workspace ws)
    {
        ...
        app.MapPost("/api/dlg", (string path, string? stamp, bool? force, DlgJson.Doc doc) =>
        {
            ...
            {
                Console.WriteLine($"[dlg] 保存失败 / save failed: {path}" + Environment.NewLine + e);
                return Results.Text(Reason(e, path), "text/plain; charset=utf-8", statusCode: 500);
            }
        });
        ...
        {
            try { return Results.Json(new { text = DialogWriter.Write(DlgJson.ToTree(doc)) }); }
            catch (Exception e)
            {
                Console.WriteLine("[dlg] 渲染失败 / render failed:" + Environment.NewLine + e);
                return Results.Text(Reason(e, null), "text/plain; charset=utf-8", statusCode: 500);
            }
        });
    }

    /// <summary>把异常翻成作者看得懂的一句话。认不出来的照抄原文——总好过一个 500。</summary>
    static string Reason(Exception e, string? path) => e switch
    {
        UnauthorizedAccessException => $"没有写入权限（文件可能是只读的）/ no write permission (read-only?): {path}",
        DirectoryNotFoundException => $"目录不在了 / folder is gone: {path}",
        IOException => $"文件被占用或写入失败 / file is in use or the write failed: {path} — {e.Message}",
        _ => e.Message,
    };
