...
public static class QuestApi
{
    ...
    static IResult Save(Workspace ws, SaveReq r)
    {
        ...
        return Results.Json(new
        {
            ...
            issues = Validate(ws, fresh, freshLoc, known),
        });
    }

    /// <summary>校验顺带带上 .dlg 里 accept:/complete: 过的任务名单（章节子任务"接得到/交得掉"两条靠它）。
    /// 没有工作区或 .dlg 扫不动就传 null，那两条规则自动跳过。</summary>
    static List<Issue> Validate(Workspace ws, QuestStore quests, LocaleStore loc, IReadOnlySet<string> known)
    {
