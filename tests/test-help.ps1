# 使用说明页的端到端测试。和 test-about.ps1 同一套路：
...
try {
  [IO.File]::WriteAllText($test,
    [IO.File]::ReadAllText("$www\index.html",$u8) + [IO.File]::ReadAllText("$sp\probe-help.js",$u8), $u8)
  ...
  $srv = Start-Process $exe -ArgumentList '--no-browser',"--root=`"$T\BepInEx\config\VisitAPI`"","--quests=`"$db`"" `
          -RedirectStandardOutput "$sp\srvhelp.log" -PassThru -WindowStyle Hidden
  try {
    $url = $null
    foreach ($i in 1..40) {
      Start-Sleep -Milliseconds 250
      $log = if (Test-Path "$sp\srvhelp.log") { Get-Content "$sp\srvhelp.log" -Raw } else { $null }
    ...
    $br = Start-Process $edge -ArgumentList '--headless=new','--disable-gpu','--window-size=2560,1400',
            "$url/ui/_help.html" -PassThru -WindowStyle Hidden
    try {
